...
class Controller(private val articleRepository: ArticleRepository) {
    ...
    @GetMapping("/articles/{arctileId}")
    fun getArticleById(@PathVariable(value = "arctileId") articleId: Long): ResponseEntity<Article> {
        return articleRepository.findById(articleId).map { article ->
            ResponseEntity.ok(article)
        }.orElse(ResponseEntity.notFound().build())
    }

    @PutMapping("/articles/{arctileId}")
    fun updateArticleById(@PathVariable(value = "arctileId") articleId: Long,
    ...
    fun deleteArticle(@PathVariable(value = "articleId") articleId: Long): ResponseEntity<Void> {
